...
public class FeedLikeController {
	...
	@PostMapping("/{feedCode}/like")
	public ResponseEntity<ApiResponse<Boolean>> toggleFeedLike(
		...
		if(!rateLimiter.isAllowed("FEED", memCode,feedCode)) {
			return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
					.body(ApiResponse.fail("요청이 너무 빠릅니다"));
		}
		
		boolean isLiked = likeService.toggleFeedLike(feedCode, memCode);
